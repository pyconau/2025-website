...
let apiBaseUrl=null
let setupRun=false
const spinStar=(star)=>{star.classList.add('fa-spin')
setTimeout(()=>{star.classList.remove('fa-spin')},400)}
const updateButton=(initial=false)=>{if(isFaved&&!favButton.querySelector('.fa-star').classList.contains('d-none'))return
...
const loadLocalFavs=()=>{const data=localStorage.getItem(`${eventSlug}_favs`)
let favs=[]
if(data){try{favs=JSON.parse(data)}catch{localStorage.setItem(`${eventSlug}_favs`,'[]')}}
return favs}
const apiFetch=async(path,method)=>{const headers={'Content-Type':'application/json'}
if(method==='POST'||method==='DELETE'){headers['X-CSRFToken']=document.cookie.split('pretalx_csrftoken=').pop().split(';').shift()}
const response=await fetch(apiBaseUrl+path,{method,headers,credentials:'same-origin',})
return response.json()}
const loadIsFaved=async()=>{if(loggedIn){return await apiFetch(`submissions/favourites/`,'GET').then(data=>{return data.includes(submissionId)}).catch(()=>{return loadLocalFavs().includes(submissionId)})}
return loadLocalFavs().includes(submissionId)}
const saveLocalFavs=()=>{let favs=loadLocalFavs()
...
const toggleFavState=async()=>{isFaved=!isFaved
saveLocalFavs()
updateButton()
if(loggedIn){await apiFetch(`submissions/${submissionId}/favourite/`,isFaved?'POST':'DELETE').catch()}}
const pageSetup=async()=>{setupRun=true
